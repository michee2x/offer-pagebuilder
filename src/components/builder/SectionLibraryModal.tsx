  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">{conf.semantic?.purpose}</p>
      </div>
    </div>
  );
});
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent
        ...
        className="w-[95vw] max-w-5xl sm:max-w-5xl md:max-w-5xl lg:max-w-5xl p-0 overflow-hidden bg-background/95 backdrop-blur-xl border-border/50 shadow-2xl sm:rounded-2xl"
      >
        <DialogClose className="absolute top-5 right-5 p-2 rounded-full hover:bg-muted/80 bg-background/50 backdrop-blur-md text-muted-foreground hover:text-foreground transition-all z-[100] border border-border/50 shadow-sm focus:outline-none">
        ...
        <div className="flex h-[80vh] max-h-[800px]">
          ...
          <div className="w-52 border-r border-border/50 bg-muted/20 p-4 shrink-0 flex flex-col gap-1.5">
            ...
            {['All Sections', 'Headers', 'Heroes', 'Features', 'Social Proof', 'Conversion'].map((label) => (
              <button
                key={label}
                className={`text-left px-3 py-2 text-sm rounded-md font-medium transition-colors ${label === 'All Sections' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'hover:bg-muted/50 text-muted-foreground'}`}
              >
                {label}
              </button>
            ))}
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 dark:bg-slate-900/20">
            <div className="mb-5">
              ...
              <p className="text-sm text-muted-foreground">Click any block to insert it into your page.</p>
            </div>
