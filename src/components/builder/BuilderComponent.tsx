...
import { ArrowUp, ArrowDown, Copy, Trash2, Edit2, Bot, ImageIcon } from 'lucide-react';
  ...
  const handleImageSelected = (url: string) => {
    ...
    if (match) {
      ...
      if (imageFields.length === 1) {
        ...
        toast.error('Could not locate the exact image prop for this component.');
      ...
      >
        <div
          className={`w-full transition-all relative ${
            !isPreviewMode ? 'border-2 border-transparent hover:border-blue-400/60' : ''
          } ${isSelected && !selectedField ? '!border-blue-500 rounded z-10' : ''} ${
            isSelected && selectedField  ? '!border-blue-300 border-dashed rounded z-10' : ''
          }`}
        >
          ...
          {!isPreviewMode && (
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 rounded">
              ...
              <div className="absolute top-2.5 right-2.5 flex gap-1 pointer-events-auto">
                {currentIndex > 0 && (
                  <button onClick={(e) => { e.stopPropagation(); moveComponent(id, currentIndex - 1); }}
                    ...
                    title="Move Up"><ArrowUp className="w-3.5 h-3.5" /></button>
                )}
                {currentIndex < rootList.length - 1 && (
                  <button onClick={(e) => { e.stopPropagation(); moveComponent(id, currentIndex + 1); }}
                    className="w-[28px] h-[28px] rounded-md bg-slate-900/95 backdrop-blur-md border border-white/10 flex items-center justify-center text-slate-400 hover:bg-blue-500/15 hover:text-blue-400 hover:border-blue-500/30 transition-all"
                    title="Move Down"><ArrowDown className="w-3.5 h-3.5" /></button>
                )}
                <button onClick={(e) => { e.stopPropagation(); duplicateComponent(id); }}
                ...
                <button onClick={(e) => { e.stopPropagation(); setSelected(id); }}
                  ...
                  title="Edit Properties"><Edit2 className="w-3.5 h-3.5" /></button>
                <button onClick={(e) => { e.stopPropagation(); removeComponent(id); }}
          ...
          {/* ── Rendered component ──────────────────────────────────────── */}
          <div ref={contentRef} className={`relative ${isHeader ? 'z-[100]' : 'z-10'}`}>
            {config.render({
              ...component.props,
              isPreviewMode,
