...
import { Loader2, Download, RefreshCw, FileText, Bot, Sparkles, Send, PenTool, Lightbulb, Target } from "lucide-react";
import { toast } from "sonner";
import { useChat } from "@ai-sdk/react";
import { DefaultChatTransport } from "ai";

export function BlueprintDashboard({ funnelId, funnelName, initialBlocks }: { funnelId: string, funnelName: string, initialBlocks: any }) {
  const [blueprintUrl, setBlueprintUrl] = useState<string | null>(initialBlocks?.blueprintUrl || null);
  ...
  async function handleGeneratePdf(topic: string) {
    ...
    const generationToast = toast.loading("Generating your Lead Magnet PDF... This might take a minute.");
    ...
    try {
      ...
      toast.success("Blueprint PDF generated successfully!", { id: generationToast });
    } catch (err) {
      console.error(err);
      toast.error("Error generating PDF. Please try again.", { id: generationToast });
    } finally {
    ...
    return (
      <main className="flex-1 overflow-hidden p-4 md:p-8 relative z-10 flex flex-col items-center justify-center bg-gradient-to-br from-[#0a0d14] to-[#131826]">
        <div className="w-full max-w-4xl flex flex-col items-center justify-center bg-white/5 border border-white/10 rounded-3xl p-12 text-center shadow-2xl backdrop-blur-md relative overflow-hidden">
           ...
           <h2 className="text-4xl font-black text-white mb-4 tracking-tight">Your Blueprint is Ready</h2>
           <p className="text-white/60 mb-10 max-w-lg text-lg leading-relaxed">
             This PDF will be automatically emailed to any leads who fill out the form on your published funnel.
           </p>
           <div className="flex gap-4">
             <Button asChild size="lg" className="h-14 bg-brand-indigo hover:bg-brand-indigo/90 text-white font-bold px-10 rounded-2xl shadow-[0_0_30px_rgba(99,102,241,0.4)] text-base">
        ...
        {hasChatStarted ? (
          <div className="flex-1 overflow-y-auto mb-6 pr-2 space-y-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-6 sticky top-0 bg-[#0a0d14]/80 backdrop-blur-xl z-20">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-xl font-bold text-white tracking-tight">Blueprint Architect</h1>
                    <p className="text-xs text-white/50 font-medium">Powered by AI & Sales Intelligence</p>
                  </div>
               </div>
               {messages.length > 1 && (
                 <Button 
                   onClick={() => handleGeneratePdf(messages[messages.length-1]?.content || "Blueprint Topic")}
                 ...
                 >
                   {isGeneratingPdf ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Sparkles className="w-4 h-4 mr-2" />}
                   Generate PDF
                 </Button>
               )}
            </div>
            ...
            {messages.map(m => (
              <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-3xl p-5 ${m.role === 'user' ? 'bg-brand-indigo text-white shadow-[0_10px_40px_-10px_rgba(99,102,241,0.5)]' : 'bg-white/5 text-white/90 border border-white/10 shadow-2xl backdrop-blur-sm'}`}>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white/5 border border-white/10 rounded-3xl p-5 flex items-center gap-3 backdrop-blur-sm">
                  ...
                  <span className="text-sm font-medium text-white/50">Architect is typing...</span>
                </div>
              </div>
            )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-lg text-white/50 mb-12 max-w-xl font-medium">
              I can analyze your funnel's sales intelligence to suggest highly-converting topics, or you can tell me exactly what you want to build.
            </p>
          </div>
        )}

        {/* Floating Input Area */}
        <div className={`w-full transition-all duration-500 ${hasChatStarted ? '' : 'max-w-2xl mx-auto'}`}>
          <form onSubmit={handleSend} className="relative group">
            ...
            <div className="relative flex items-center bg-[#131826] border border-white/10 rounded-3xl shadow-2xl overflow-hidden p-2 pl-4">
              <input
                ...
                placeholder={hasChatStarted ? "Reply to the architect..." : "Describe the blueprint you want to create..."}
          ...
          {!hasChatStarted && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              {[
                { icon: Lightbulb, title: "Suggest topics", desc: "Based on my funnel intelligence" },
                { icon: PenTool, title: "Checklist", desc: "Create a step-by-step checklist" },
                { icon: Target, title: "Action Plan", desc: "Write a 30-day action plan" },
                { icon: FileText, title: "Resource Guide", desc: "Curate a list of top tools" },
              ].map((card, i) => (
                <button
                  key={i}
                  onClick={() => {
                    setInputValue(card.title);
                    append({ role: "user", content: `${card.title}. ${card.desc}.` });
                  }}
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="font-bold text-white/90">{card.title}</span>
                  </div>
                  <span className="text-sm text-white/40 font-medium">{card.desc}</span>
                </button>
              ))}
            </div>
          )}
