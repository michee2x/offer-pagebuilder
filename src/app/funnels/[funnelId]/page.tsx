...
async function fetchPostHogStats(pageId: string) {
  ...
  // Tier classification
  const T1 = new Set(["United States","United Kingdom","Canada","Australia","New Zealand"]);
  const T2 = new Set(["Germany","France","Spain","Italy","Netherlands","Sweden","Switzerland","Norway","Denmark","Belgium","Austria","Finland"]);
  ...
  const pageBreakdown = PAGE_ORDER
    .filter(p => pageMap[p] !== undefined)
    .map(p => ({
      path:  p,
  ...
  return {
    views7d, visitors7d, viewsToday,
    viewsPrev7d, visitorsPrev7d,
    countries,
    tiers: tiers.length ? tiers : [{ name: "No Data", value: 1, color: "#64748b" }],
    desktopViews, mobileViews, tabletViews,
    recentTraffic,
    pageBreakdown,
  };
}
...
export default async function FunnelDashboardPage({ params }: Props) {
  ...
  const dashboardData = {
    ...
    uniqueVisitorsChange: pct(analytics?.visitors7d ?? 0, analytics?.visitorsPrev7d ?? 0),
    desktopViews:  analytics?.desktopViews ?? 0,
    mobileViews:   analytics?.mobileViews  ?? 0,
    tabletViews:   analytics?.tabletViews  ?? 0,
    tiers:         analytics?.tiers        ?? [{ name: "No Data", value: 1, color: "#64748b" }],
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#0a0a0f]">
            <div className="w-full max-w-7xl mx-auto space-y-8">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-black tracking-tight text-white/90">{funnel.name}</h1>
