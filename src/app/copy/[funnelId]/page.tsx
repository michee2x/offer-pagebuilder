          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(
                ref.current?.innerText || localContent,
              );
    ...
    <div>
      <div className="flex items-start justify-between mb-4">
        <div>
          <div
            className={cn(
              "inline-flex text-[10px] font-bold px-2 py-0.5 rounded-full border mb-1.5",
              badge,
            )}
          >
            {pageLabel}
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {pageCopy.sections.length} sections · {pageCopy.word_count} words ·
            Score {pageCopy.score}
          </p>
        </div>
        <button
          onClick={() => {
            const all = pageCopy.sections
              .map((s) => `[${s.label}]\n${s.content}`)
              .join("\n\n---\n\n");
            navigator.clipboard
              .writeText(all)
              .then(() => toast.success("All copy copied!"));
          }}
...
export default function CopyPage({
  ...
  const overallScore = copy
    ? Math.round(
        [copy.lead_capture, copy.sales_page, copy.upsell, copy.thankyou].reduce(
          (a, p) => a + (p?.score || 0),
          0,
        ) / 4,
      )
          ...
          <div className="w-52 shrink-0 border-r border-border bg-card flex flex-col overflow-hidden">
            ...
            <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
              {PAGE_NAV.map((page) => {
                ...
                const score =
                  page.key !== "email_sequence"
                    ? (pageCopy as PageCopy)?.score
                    : 0;
              ...
              {isInitializing ? (
                ...
                activePage === "email_sequence" ? (
                  ...
                  <PageWorkspace
                    ...
                    pageLabel={
                      PAGE_NAV.find((p) => p.key === activePage)?.label ||
                      activePage
                    }
              ...
              {copy && (
                <div className="bg-muted/30 border border-border rounded-xl p-4">
                  ...
                  <div className="space-y-2">
                    {PAGE_NAV.filter((p) => p.key !== "email_sequence").map(
                      (page) => {
